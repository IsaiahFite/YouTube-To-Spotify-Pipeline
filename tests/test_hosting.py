...
@patch.dict("os.environ", {"GITHUB_TOKEN": "test_token", "GITHUB_REPO": "test/repo"})
@patch("src.hosting.Github")
def test_upload_audio_already_exists(mock_github):
    """Test skipping upload when asset already exists on release"""
    mock_existing_asset = MagicMock()
    mock_existing_asset.name = "audio.mp3"
    mock_existing_asset.browser_download_url = "https://github.com/download/existing"
    ...
    result = upload_audio("audio.mp3")
    assert result == "https://github.com/download/existing"
    mock_release.upload_asset.assert_not_called()
